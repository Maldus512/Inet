...
simsignal_t Ieee80211AgentSTA::opplineSignal = registerSignal("oppline_signal");
...
void Ieee80211AgentSTA::setProbeDelay(simtime_t probe) {
    probeDelay = probe;
}
...
{
    // choose best AP
    if ((int)resp->getBssListArraySize() > 0) {
        emit(opplineSignal, PR_SCAN_CONFIRM);
    }
    int bssIndex = -1;
